...
1. Используй инструмент list_models_for_role для роли `admin`:
   - Получи список доступных моделей с рекомендациями
   - Запиши результаты в ~/agent-config/model_analysis.md
2. Используй get_agent_info для агента `admin`:
   - Запиши текущие настройки агента
   - Сохрани в ~/agent-config/current_config.md
3. Проанализируй рекомендации и выбери оптимальную модель для Admin:
   - Предпочтительно: модель с поддержкой tools и достаточным размером (7B+)
   - Если модель компактная (≤3B): ориентируйся на составные навыки и простые задачи
4. Используй configure_agent для настройки агента `admin`:
   - Установи выбранную модель
   - Установи провайдера (ollama для локальных)
5. Проверь настройки повторно через get_agent_info
6. Проверь работу tool calling на Admin (минимум 3 инструмента):
   - list (например, /tmp)
   - sysinfo
   - execute (например, uname -a)
7. Сохрани итоговый отчёт в ~/agent-config/setup_report.md:
   - Какая модель выбрана и почему
   - Результаты проверки инструментов
